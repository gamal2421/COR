...
class archivedt extends StatefulWidget {
  final Map<String, dynamic> cv;
  const archivedt({super.key, required this.cv});

  @override
  State<archivedt> createState() => _archivedtState();
}

class _archivedtState extends State<archivedt> {
        ...
        children: [
          ...
          FloatingActionButton(
            ...
            onPressed: () async {
              try {
                final documentId = widget.cv['id'];
                // Copy the document from the Archive collection back to the CV collection
                await Firestore.instance
                    .collection('CV')
                    .document(documentId)
                    .set(widget.cv);
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('CV de-archived successfully'),
                    backgroundColor: Colors.green,
                    duration: Duration(seconds: 2),
                  ),
                );

                // Navigate to the main CV list (FireStoreHome)
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text('Error de-archiving CV: $e'),
                    backgroundColor: Colors.red,
                  ),
                );
              }
  ...
  Widget _buildValueWidget(BuildContext context, dynamic value) {
    if (value is List) {
      return Column(
        ...
        mainAxisSize: MainAxisSize.min,
        children:
            value.map<Widget>((item) => _buildListItem(context, item)).toList(),
      );
    } else if (value is Map) {
      return _buildMapWidget(context, value);
    } else {
      return _buildValueText(context, value.toString());
    }
  }
  ...
  Widget _buildListItem(BuildContext context, dynamic item) {
    if (item is Map) {
      return Padding(
        ...
        child: _buildMapWidget(context, item),
      );
    } else {
      return Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          Expanded(child: _buildValueText(context, item.toString())),
        ],
      );
    }
  }

  Widget _buildMapWidget(BuildContext context, Map map) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
